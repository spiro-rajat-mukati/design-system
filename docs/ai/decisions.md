...
**D11 · Assets follow an assets-by-type model; icons live in `@kijani/icons`.** _(locked 2026-06-15)_ Design assets (icons / illustrations / images) are not stored inside the component packages. They live in a dedicated `packages/icons` workspace (`@kijani/icons`) that mirrors the tokens source→build pattern: authored SVG sources in `svg/`, generated React (web) and react-native-svg (native) components in `src/`, regenerated by a zero-dep `build.mjs`. Entry points: `@kijani/icons` (web barrel), `@kijani/icons/native` (RN barrel), deep per-icon imports for tree-shaking. CI mirrors tokens: `icons-build.yml` (path-filtered to `svg/**`, PAT-signed commit) + `icons-validate.yml` (no path filter, idempotency guard). Illustrations and images will follow the same pattern in their own packages when the time comes. The Figma export step (future plugin action) writes to `packages/icons/svg/` only — never touches generated files.
