...
/* ── Ensure Text Styles — role tables ── */
...
const WEB_STYLE_NAMES = {
  "display-l": "Web/Display/Large",  "display-m": "Web/Display/Medium", "display-s": "Web/Display/Small",
  "heading-1": "Web/Heading/1",      "heading-2": "Web/Heading/2",      "heading-3": "Web/Heading/3",
  "heading-4": "Web/Heading/4",      "heading-5": "Web/Heading/5",      "heading-6": "Web/Heading/6",
  "body-l":    "Web/Body/Large",     "body-m":    "Web/Body/Medium",    "body-s":    "Web/Body/Small",
  "label-l":   "Web/Label/Large",    "label-m":   "Web/Label/Medium",   "label-s":   "Web/Label/Small",
  "caption":   "Web/Caption",        "code":      "Web/Code",           "overline":  "Web/Overline",
};
...
/* ============================================================
   ENSURE TEXT STYLES
   Imports Foundations published semantic vars into the active
   (Web or Mobile) file and binds each Text Style sub-property.
   ============================================================ */

/** Find the "Design System" collection in available team libraries.
 *  Returns { byName: Map<varName, LibraryVariable> } or { error }. */
async function findLibraryVars() {
  ...
  const dsCol = collections.find(function(c) { return c.name === COLLECTION_NAME; });
  if (!dsCol) {
...
/** Return the canonical variable names for one role on a given platform. */
...
async function computeEnsureTextStylesPreview(platform) {
  ...
  const existingStyles = figma.getLocalTextStyles();
  const styleByName = new Map(existingStyles.map(function(s) { return [s.name, s]; }));
  ...
  for (const role of roles) {
    ...
    if (!byName.has(vn.size) || !byName.has(vn.weight) || !byName.has(vn.family)) {
      ...
      missingVars.push({ styleName: styleName, missing: missing });
      continue;
    }
    ...
    rows.push({ role: role, styleName: styleName, action: action, varNames: vn });
  }
...
async function applyEnsureTextStyles(platform) {
  ...
  for (const role of roles) {
    ...
    // Idempotency: if already bound to the right size var, skip
    ...
    if (existingStyle) {
      ...
      if (bvFs) {
        const bound = figma.variables.getVariableById(bvFs.id);
        if (bound && bound.name === vn.size) { skipped++; continue; }
      }
    }

    // Import from Foundations library
    ...
    // Resolve literal values for fontName / fontSize / lineHeight (set before binding)
    let family   = "Inter";
    ...
    await figma.loadFontAsync({ family: family, style: styleStr }).catch(function() {});
    ...
    if (spacingImported) {
      ...
      if (typeof spacingVal === "string" && spacingVal.endsWith("em")) {
        ...
        if (isFinite(n)) try { existingStyle.letterSpacing = { value: n * 100, unit: "PERCENT" }; } catch (e) {}
      }
    }

    // Bind variables
    try { existingStyle.setBoundVariable("fontFamily", familyImported); } catch (e) { uiLog("  · " + styleName + " fontFamily: " + e.message, "warn"); }
    try { existingStyle.setBoundVariable("fontStyle",  weightImported); } catch (e) { uiLog("  · " + styleName + " fontStyle: "  + e.message, "warn"); }
    try { existingStyle.setBoundVariable("fontSize",   sizeImported);   } catch (e) { uiLog("  · " + styleName + " fontSize: "   + e.message, "warn"); }
    if (lineImported) {
      try { existingStyle.setBoundVariable("lineHeight", lineImported); } catch (e) { uiLog("  · " + styleName + " lineHeight: " + e.message, "warn"); }
    }

    if (isNew) { created++; uiLog("  · Created: " + styleName, "ok"); }
    else       { updated++; uiLog("  · Updated: " + styleName, "ok"); }
  }
  ...
  figma.ui.postMessage({ type: "ensure-text-styles-done", created: created, updated: updated, skipped: skipped, failures: failures });
}
...
/* ============================================================
   PRUNE — delete Figma-only (orphaned) variables
   Reuses computeDiff's "in Figma, not in repo" set.
   Checks each candidate for alias references from other variables
   and text style bindings before marking it safe to delete.
   ============================================================ */

async function computePrunePreview(repoData) {
  var collection = figma.variables.getLocalVariableCollections()
    .find(function(c) { return c.name === COLLECTION_NAME; });
  if (!collection) {
  ...
  var removedNames = new Set(diff.removed.map(function(r) { return r.name; }));

  // Build id → name index across ALL local variables (not just this collection)
  // so we can detect cross-collection alias references too.
  ...
  // Find removed vars that are still aliased by other variables.
  var aliasedBy = new Map();
  ...
  // Find removed vars bound to local text styles.
  var boundInStyle = new Map();
  ...
  return { canDelete: canDelete, willSkip: willSkip, total: diff.removed.length };
}
...
function applyPrune(names) {
  ...
  for (var ni = 0; ni < names.length; ni++) {
    ...
    if (!variable) {
      errors++;
      uiLog("  · Not found: " + name, "warn");
      continue;
    }
    try {
      variable.remove();
      deleted++;
      uiLog("  · Deleted: " + name, "muted");
    } catch (e) {
      errors++;
      uiLog("  · Cannot delete " + name + ": " + (e && e.message ? e.message : String(e)), "warn");
    }
  }
  ...
  figma.ui.postMessage({ type: "prune-done", deleted: deleted, errors: errors });
}
...
/* ============================================================
   PUSH — serialize the local Figma variable collection so ui.js
   can diff it against the GitHub source files and create a PR.
   All Figma API calls live here; network calls live in ui.js.
   ============================================================ */

/** Convert Figma RGBA float object to lowercase hex string.
 *  Alpha is omitted when fully opaque. */
function figmaColorToHex(c) {
  ...
  return (Math.round((c.a !== undefined ? c.a : 1) * 255) < 255) ? hex + toHex(c.a) : hex;
}

/**
 * Read the local "Design System" variable collection and return a plain-object
 * representation that can be sent to ui.js via postMessage.
 *
 * Values are normalised:
 *  - COLOR → lowercase hex string (#rrggbb or #rrggbbaa)
 *  - VARIABLE_ALIAS → { alias: "target/variable/name" } (Figma slash path)
...
function readFigmaCollection() {
  ...
  var modeById = {};
  collection.modes.forEach(function(m) { modeById[m.modeId] = m.name; });

  var variables = [];
  var idToName = {}; // pre-index variable names for alias lookup
  ...
  collection.variableIds.forEach(function(id) {
    ...
    variables.push({ name: v.name, type: v.resolvedType, values: values });
  });
...
/* ============================================================
   COMPONENT DRIFT SCAN (Phase B + C)
   Phase B — extracts COMPONENT_SET variant/property metadata.
   Phase C — audits each set's primary variant for bindable visual
...
function _layerPath(node, stopAt) {
  var parts = [];
  var cur = node;
  while (cur && cur !== stopAt) { parts.unshift(cur.name); cur = cur.parent; }
  return parts.join(" > ");
}
...
function auditHardcodedValues(componentSet) {
  ...
  if (componentSet.children) {
    for (var i = 0; i < componentSet.children.length; i++) {
      if (componentSet.children[i].type === "COMPONENT") { primary = componentSet.children[i]; break; }
    }
  }
...
function scanComponentDrift() {
  ...
  figma.root.findAll(function(n) { return n.type === "COMPONENT_SET"; }).forEach(function(set) {
    ...
    var lint = [];
    try { lint = auditHardcodedValues(set); } catch (_) {}
    sets.push({ name: set.name, figmaProps: figmaProps, lint: lint });
  });
...
figma.ui.onmessage = async (msg) => {
  try {
    if (msg.type === "compute-diff") {
      ...
      // Push step 1: serialize the Figma collection and hand it to the UI.
      ...
      figma.ui.postMessage({ type: "ensure-ts-preview-result", preview: preview });
      ...
      figma.ui.postMessage({ type: "prune-preview-result", preview: preview });
