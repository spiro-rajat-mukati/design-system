  ...
  switch (msg.type) {
    ...
    case "component-drift-scanned": {
      (async () => {
        ...
        log("Component Drift (" + esc(platform) + " \u2014 " + esc(msg.fileName || "") + "): " + s.coverage + " coverage, " + s.parity + " parity, " + s.total + " total", s.total > 0 ? "warn" : "ok");
        setBusy(false);
      })().catch(function(e) {
...
function computeComponentDrift(figmaComponents, codeManifest) {
  ...
  return { issues, summary: { coverage: coverageCount, parity: parityCount, total: issues.length } };
}
