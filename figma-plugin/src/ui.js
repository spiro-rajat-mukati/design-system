...
/* ── localStorage prefs (tab + window size; no main-thread roundtrip needed) ── */
function savePrefs(p) { try { localStorage.setItem('ds-ui-prefs', JSON.stringify(p)); } catch (_) {} }
function loadPrefs() { try { return JSON.parse(localStorage.getItem('ds-ui-prefs') || '{}'); } catch (_) { return {}; } }
...
/* ── Context banner ── */
var FILE_CTX = [
  { re: /foundation/i, tab: 'tokens',     tabs: ['tokens','styles','components'],    label: 'tokens · styles · components' },
...
function updateContextBanner(fileName) {
  ...
  if (!banner || !fileName) { if (banner) banner.hidden = true; return; }
  var matched = null;
  for (var i = 0; i < FILE_CTX.length; i++) {
    if (FILE_CTX[i].re.test(fileName)) { matched = FILE_CTX[i]; break; }
  }
  if (matched) {
...
/* ── Info button → open / close info panel ── */
(function() {
  ...
  if (infoBtn && infoPanel) {
    infoBtn.addEventListener('click', function() { infoPanel.hidden = !infoPanel.hidden; });
  }
...
/* ── Drag handle — resize plugin window ── */
(function() {
...
/* ── Restore persisted prefs (tab + size) ── */
(function() {
  var prefs = loadPrefs();
