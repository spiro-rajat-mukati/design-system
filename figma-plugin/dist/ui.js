...
function optimizeSVG(svgStr) {
  ...
  const neutral = /* @__PURE__ */ new Set(["none", "transparent", "currentcolor", "inherit"]);
...
function sha1PureJs(data) {
  ...
  msg[ml] = 128;
  const bl = ml * 8 >>> 0;
  msg[totalLen - 4] = bl >>> 24 & 255;
  msg[totalLen - 3] = bl >>> 16 & 255;
  msg[totalLen - 2] = bl >>> 8 & 255;
  msg[totalLen - 1] = bl & 255;
  const K = [1518500249, 1859775393, 2400959708 | 0, 3395469782 | 0];
...
function renderAssetLane(laneTitle, ext, diff) {
  ...
  if (errors.length) {
    ...
    html += errors.map(
      (i) => '<div class="ei-row"><span class="var-name removed ei-name">' + esc(i.name) + '</span><span class="ei-warn">' + esc(i.exportError || "Unknown error") + "</span></div>"
    ).join("");
    html += "</details>";
  }
  html += "</div>";
...
document.getElementById("export-assets-confirm").addEventListener("click", async () => {
  ...
  try {
    ...
    logEl2.insertAdjacentHTML(
      "beforeend",
      '<div class="line ok">PR: <a class="pr-link" href="' + esc(pr.html_url) + '" target="_blank">' + esc(pr.html_url) + "</a></div>"
    );
    logEl2.scrollTop = logEl2.scrollHeight;
  } catch (e) {
  ...
  switch (msg.type) {
    ...
    case "component-drift-scanned": {
      (async () => {
        ...
        log(
          "Component Drift (" + esc(platform) + " \u2014 " + esc(msg.fileName || "") + "): " + s.coverage + " coverage, " + s.parity + " parity, " + s.total + " total" + lintNote,
...
function computeComponentDrift(figmaComponents, codeManifest) {
  ...
  var figmaByName = {};
  figmaComponents.forEach(function(c) {
    figmaByName[norm(c.name)] = c;
  });
  var codeByName = {};
  (codeManifest.components || []).forEach(function(c) {
    codeByName[norm(c.name)] = c;
  });
  Object.keys(figmaByName).forEach(function(k) {
    if (!codeByName[k]) {
      ...
      issues.push({
        ...
        category: "figma-only",
        component: name,
        noise: INTERNAL_HELPER_NAMES.indexOf(norm(name)) !== -1
      });
    }
  });
    ...
    Object.keys(fProps).forEach(function(pk) {
      ...
      if (!cProps[pk]) {
        ...
        issues.push({
          kind: "parity",
          severity: "warning",
          category: "figma-prop-not-in-code",
          component: cc.name,
          prop: fp.original,
          figmaType: fp.type,
      ...
      if (fp.type === "VARIANT" && fp.options && cProps[pk].kind === "union" && cProps[pk].options) {
        var fOpts = {};
        fp.options.forEach(function(o) {
          fOpts[norm(o)] = o;
        });
        var cOpts = {};
        cProps[pk].options.forEach(function(o) {
          cOpts[norm(o)] = o;
        });
        Object.keys(fOpts).forEach(function(o) {
          if (!cOpts[o]) {
            issues.push({
              kind: "parity",
              severity: "warning",
              category: "option-missing-in-code",
              component: cc.name,
              prop: fp.original,
              option: fOpts[o],
              noise: false
        ...
        Object.keys(cOpts).forEach(function(o) {
          if (!fOpts[o]) {
            issues.push({
              kind: "parity",
              severity: "info",
              category: "option-missing-in-figma",
              component: cc.name,
              prop: fp.original,
              option: cOpts[o],
              noise: false
    ...
    Object.keys(cProps).forEach(function(pk) {
      ...
      if (!fProps[pk]) {
        ...
        issues.push({
          kind: "parity",
          severity: "info",
          category: "code-prop-not-in-figma",
          component: cc.name,
          prop: cp.original,
          codeKind: cp.kind,
          noise: noiseByKind || noiseByName
        });
      }
    });
  });
...
function renderCoverageRow(i, isNoise) {
  var label = i.category === "figma-only" ? "In Figma, not in code library" : i.category === "code-only" ? "In code, no Figma component set" : i.category === "no-code-connect" ? "No Code Connect configured" : i.category;
  var cls = "drift-row" + (isNoise ? " noise" : "");
  return '<div class="' + cls + '"><div class="drift-comp">' + esc(i.component) + '</div><div class="drift-issue">' + severityBadge(i.severity) + " " + esc(label) + "</div></div>";
}
...
function renderDriftReport(result, lintFindings) {
  ...
  var realLint = lint.filter(function(f) {
    return !f.noise;
  }).length;
  ...
  html += "</div>";
  var covIssues = issues.filter(function(i) {
    return i.kind === "coverage";
  });
  var realCovIssues = covIssues.filter(function(i) {
    return !i.noise;
  });
  var noiseCovIssues = covIssues.filter(function(i) {
    return i.noise;
  });
  if (covIssues.length > 0) {
    html += '<div class="drift-section">';
    html += '<div class="drift-section-title">Coverage (' + realCovIssues.length + (noiseCovIssues.length > 0 ? " real" : "") + ")</div>";
    realCovIssues.forEach(function(i) {
      html += renderCoverageRow(i, false);
    });
    if (noiseCovIssues.length > 0) {
      html += '<details class="drift-minor"><summary>Show ' + noiseCovIssues.length + " minor</summary>";
      noiseCovIssues.forEach(function(i) {
        html += renderCoverageRow(i, true);
      });
      html += "</details>";
    }
    html += "</div>";
  }
  var parIssues = issues.filter(function(i) {
    return i.kind === "parity";
  });
  var realParIssues = parIssues.filter(function(i) {
    return !i.noise;
  });
  var noiseParIssues = parIssues.filter(function(i) {
    return i.noise;
  });
  if (parIssues.length > 0) {
    html += '<div class="drift-section">';
    html += '<div class="drift-section-title">Prop / variant parity (' + realParIssues.length + (noiseParIssues.length > 0 ? " real" : "") + ")</div>";
  ...
  var realLintItems = lint.filter(function(f) {
    return !f.noise;
  });
  var noiseLintItems = lint.filter(function(f) {
    return f.noise;
  });
  if (lint.length > 0) {
    html += '<div class="drift-section">';
    html += '<div class="drift-section-title">Hardcoded values (' + realLintItems.length + (noiseLintItems.length > 0 ? " real" : "") + ")</div>";
